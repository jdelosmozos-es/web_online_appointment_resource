...
class MemberAppointment(http.Controller):
    ...
    @http.route('/appointment/member/<int:space_id>/<int:calendar_id>/<int:num_persons>', auth="public", website=True)
    def calendar(self, space_id=None, day=None, calendar_id=None, selectedDate=None, num_persons=None, **post):
        calendar = request.env['web.online.appointment'].sudo().browse(int(calendar_id))
        today = datetime.now()
        cal_lines = request.env['web.online.appointment.line'].sudo().search([('line_id', '=', calendar.id),
    ...
    def confirm_booking(self, **post):
        ...
        if post.get('calendar_id'):
            ...
            calendar_id = request.env['web.online.appointment'].sudo().browse(int(post.get('calendar_id')))
...
#                'name': '%s-%s' % (post.get('first_name'), post.get('start_datetime')),
            ...
            domain = [('line_id', '=', int(post.get('calendar_id'))), ('start_datetime', '>=', fields.Datetime.to_string(utc_date)), ('start_datetime', '<', fields.Datetime.to_string(stop_date))]
    ...
    def get_time_slots(self, calendar_id, selectedDate, num_persons):
        ...
        year = sel_date[2]
        today = datetime.now()
        calendar1 = request.env['web.online.appointment'].sudo().browse(int(calendar_id))
        event_duration_minutes = calendar1.space.event_duration_minutes
        Line = request.env['web.online.appointment.line']
        calendar_lines = Line.sudo().search([('line_id', '=', int(calendar_id)),('start_datetime', '>=', datetime.now())])
        ...
        for line in calendar_lines:
            ...
            if event_lines and min(event_lines.mapped('availability')) > int(num_persons):
                available_lines |= line
#        calendar_line = request.env['web.online.appointment.line'].sudo().search([('line_id', '=', int(calendar_id))])
        slots = []
